...
angular.module('bahmni.common.services')
    .factory('pacsService', ['$http', '$filter', function ($http, $filter) {
        ...
        var mapToFullfilledOrder = function (study) {
            ...
            if (!fo.label) { fo.label = getDcmValue(study, "00400255", ""); }
            if (!fo.label) { fo.label = getDcmValue(study, "00081030", ""); }
            if (!fo.label) { fo.label = getDcmValue(study, "00321060", "image"); }
            fo.provider = '';
             ...
             *  time.
             *
             * Another solution: prefix accession numbers in a way that
             * those which match a Bahmni order can be identified by their
             * prefix. Ie, normal accession numbers would be 5,6,7,8, and
             * those from a Bahmni order would be BAH5, BAH432, etc. Then
             * matching can occur on the accession number, but x-rays
             * generated without an order wouldn't match.
             *
             * Another option: see if MWL is a solution.
             */
